...
namespace MonitorSistema
{
    public class LectorRecursosSistema
    {
        ...
        bool compacto = false;
        private RecursosSistema recursos;

        public LectorRecursosSistema(RecursosSistema recursos)
        {
            ...
            recursos.Usuario = getUsuario();
            recursos.Fabricante = getOrdenadorFabricante();
            recursos.Modelo = getOrdenadorModelo();
            recursos.Procesador = getProcesadores()[0];
            recursos.DiscosLogicos = getDiscosLogicos();
        }

        public void actualiza()
        {
            recursos.DiscoLectura.Valor = getDatosDisco(DiskData.Read);
            recursos.DiscoEscritura.Valor = getDatosDisco(DiskData.Write);
            recursos.DiscoLecturaEscritura.Valor = getDatosDisco(DiskData.ReadAndWrite);

            recursos.MemoriaFisicaDisponible.Valor = getMemoriaFisicaDisponible();
            recursos.MemoriaFisicaOcupada.Valor = getMemoriaFisicaOcupada();
            recursos.MemoriaVirtualDisponible.Valor = getMemoriaVirtualDisponible();
            recursos.MemoriaVirtualOcupada.Valor = getMemoriaVirtualOcupada();
            recursos.MemoriaFisicaPorcentaje.Valor = recursos.MemoriaFisicaOcupada.Valor / recursos.MemoriaFisicaDisponible.Valor * 100;
            recursos.MemoriaVirtualPorcentaje.Valor = recursos.MemoriaVirtualOcupada.Valor / recursos.MemoriaVirtualDisponible.Valor * 100;

            if (recursos.MemoriaFisicaPorcentaje.Valor > recursos.MemoriaFisicaPorcentajeMaximo.Valor)
            {
                recursos.MemoriaFisicaPorcentajeMaximo.Valor = recursos.MemoriaFisicaPorcentaje.Valor;
            }

            if (recursos.MemoriaVirtualPorcentaje.Valor > recursos.MemoriaVirtualPorcentajeMaximo.Valor)
            {
                recursos.MemoriaVirtualPorcentajeMaximo.Valor = recursos.MemoriaVirtualPorcentaje.Valor;
            }

            recursos.RedPaquetesEntrada.Valor = getDatosRed(NetData.Received);
            recursos.RedPaquetesSalida.Valor = getDatosRed(NetData.Sent);
            recursos.RedPaquetesEntradaSalida.Valor = getDatosRed(NetData.ReceivedAndSent);
        }
        ...
        public double getMemoriaFisicaOcupada()
        {
            return GetCounterValue(memoria, "Memory", "Available Bytes", null); 
        }

        public double getMemoriaVirtualDisponible()
        {
           return GetCounterValue(memoria, "Memory", "Commit Limit", null);

        }

        public double getMemoriaVirtualOcupada()
        {
