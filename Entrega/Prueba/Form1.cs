...
namespace Prueba
{
    public partial class form1 : Form
    {
        ...
        private void anyadirNumDisplay(string num) 
        {
            ...
            try
            {
                ...
                if (!text.Contains(','))
                {
                    ...
                    lblDisplay.Text += num;
                if (opActual.Text.Equals(""))
                    opActual.Text = lblDisplay.Text;
                else
                    opActual.Text += " " + lblDisplay.Text;
        ...
        // Operaciones + - x / 
        private void gestionarOperacion(Operaciones op, string signo) {
            operando1 = float.Parse(lblDisplay.Text);
            operacion = op;
            lblDisplay.Text = "0";
            opActual.Text = operando1 + " " + signo;
            btnResult.Enabled = true;
        }
        private void btnSuma_Click(object sender, EventArgs e)
        {
            gestionarOperacion(Operaciones.Suma, "+");
        }

        private void btnResta_Click(object sender, EventArgs e)
        {
            gestionarOperacion(Operaciones.Resta, "-");
        }

        private void btnMultiplica_Click(object sender, EventArgs e)
        {
            gestionarOperacion(Operaciones.Producto, "X");
        }


        private void btnDivide_Click(object sender, EventArgs e)
        {
            gestionarOperacion(Operaciones.Division, "/");
        }
        ...
        {//potencia 
            gestionarOperacion(Operaciones.Potencia, "^");
        }
        ...
        {//Raíz cuadrada.
            btnResult.Enabled = true;
            operando1 = float.Parse(lblDisplay.Text);
            double res = Math.Sqrt(operando1);
            lblDisplay.Text = Convert.ToString(res);
            opActual.Text = "\u221A" + operando1 + " = " + lblDisplay.Text;
        }
